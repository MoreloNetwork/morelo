...
def build():
    ...
    os.makedirs('morelo-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../morelo/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'morelo='+args.commit, '--url', 'morelo='+args.url, '../morelo/contrib/gitian/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win', '--destination', '../gitian.sigs/', '../morelo/contrib/gitian/gitian-win.yml'])
        subprocess.check_call('mv build/out/morelo*.zip ../morelo-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'morelo='+args.commit, '--url', 'morelo'+args.url, '../morelo/contrib/gitian/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx', '--destination', '../gitian.sigs/', '../morelo/contrib/gitian/gitian-osx.yml'])
        subprocess.check_call('mv build/out/morelo*.tar.gz ../morelo-binaries/'+args.version, shell=True)
...
def verify():
    ...
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../morelo/contrib/gitian/gitian-linux.yml'])
    print('\nVerifying v'+args.version+' Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win', '../morelo/contrib/gitian/gitian-win.yml'])
    print('\nVerifying v'+args.version+' MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx', '../morelo/contrib/gitian/gitian-osx.yml'])
    os.chdir(workdir)
...
def main():
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/morelo')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
