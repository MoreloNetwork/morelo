...
Core tests take longer than any other tests, due to the high amount of computational work involved in validating core components.
...
To run only core tests (after building):
...
To run only crypto tests (after building):
...
To run only hash tests (after building):
...
To run only performance tests (after building):
...
To run only unit tests (after building):
